...
class Juego():
    ...
    def game_over(self):  # BASE DE DATOS
        ...
        if conn and hasattr(self, "usuario_actual"):
            try:
                cursor = conn.cursor()

                # Obtener el id_usuario desde la tabla
                cursor.execute("SELECT id_usuario FROM usuarios WHERE usuario = ?", (self.usuario_actual,))
                ...
                if resultado:
                    ...
                    # Insertar el puntaje en la tabla puntajes
                    cursor.execute("INSERT INTO puntajes (id_usuario, puntaje, fecha) VALUES (?, ?, ?)",
                                (id_usuario, self.puntaje, fecha_actual))
                    conn.commit()
